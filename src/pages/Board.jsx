...
const Board = () => {
  ...
  const handleXmlUpload = async (e) => {
    ...
    try {
      ...
      if (/\.eap$/i.test(file.name)) {
        ...
        Swal.close();
        if (!res.ok) throw new Error(cuerpo?.message || 'No se pudo leer el proyecto .EAP');
  ...
  const createBoardFromXml = async (title, nodes, edges) => {
    try {
      ...
      const result = await res.json().catch(() => ({}));
      if (!res.ok) {
        const msg = result?.message || result?.data?.message || 'No se pudo crear el tablero importado';
        throw new Error(msg);
      }
      ...
      Swal.fire({ icon: "error", title: "Error", text: error.message || "No se pudo crear el tablero" });
    }
  };
