...
export default function AiBubble({ boardId, nodes, edges, setNodes, setEdges, updateBoardData, onAiModificationChange }) {
  ...
  const ejecutarLocal = async (texto) => {
    ...
    try {
      ...
      const lineas = [];
      if (r.cambios > 0) {
        lineas.push(`💻 Hecho con la IA local (${r.motor === 'modelo' ? 'modelo de lenguaje' : 'intérprete de órdenes'}):`);
      ...
      pushMessage({ role: 'ai', text: `La IA local falló: ${err.message}` });
    } finally {
      setLoading(false);
    }
  };
  ...
  const enviarVoz = async () => {
    ...
    try {
      ...
      pushMessage({ role: 'ai', text: `No se pudo transcribir el audio: ${err.message}` });
      setLoading(false);
      return;
    }
  ...
  const handleSendNube = async (textoForzado = null, modoForzado = null) => {
    ...
    try {
      ...
      if (mode === 'image' || !text) {
        ...
        pushMessage({ role: 'ai', text: `La IA en la nube no respondió (${err.message}). Lo resuelvo con la IA local…` });
        setLoading(false);
        await ejecutarLocal(text);
      }
    } finally {
